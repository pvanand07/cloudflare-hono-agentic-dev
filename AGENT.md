...
Hono on Cloudflare Workers, `wrangler dev` for local development (no Vite), JSX pages via `hono/jsx-renderer`, D1 + Drizzle for data, `@hono/zod-validator` for request validation. See `docs/architecture.md`.
...
| Migrations | `migrations/`, generated by `drizzle-kit generate` |
| Static assets | `public/` |

`docs/agents/issue-tracker.md` and `docs/agents/domain.md` are stubs until `/setup-matt-pocock-skills` has been run once (see `SETUP.md`, step 13) — don't treat their placeholder content as verified config before that.
...
2. Run `npx drizzle-kit generate` to produce a migration.
3. Run `npx wrangler d1 migrations apply <db-name> --local` for local dev, `--remote` for production.

## Keeping this stable

- Dependency versions in `package.json` are pinned exactly, not with `^`/`~`. Don't reintroduce ranges without a reason.
- No test runner is included — none was found working reliably against this stack at the time of writing. Verify changes with `wrangler dev` and manual requests.
- Migrate locally first, always. Treat `--remote` as a deliberate, separate action.
- Rerun `npm run cf-typegen` after every `wrangler.jsonc` change, before touching bindings in code.
